...
describe("Should create New Paste in website Pastebin.com with next params", () => {
  before(async () => {
    Pastebin.open();
    await Pastebin.createCode
      .addValue(`git config --global user.name  "New Sheriff in Town"
    ...
    await Pastebin.openPasteName.addValue(
  ...
  it("Should check code", async () => {
    await expect(Pastebin.checkCode)
      .toHaveValue(`git config --global user.name  "New Sheriff in Town"
      //git reset $(git commit-tree HEAD^{tree} -m "Legacy code")
     //git push origin master --force`);
  });

  it("Should check Syntax Highlighting ", async () => {
    await expect(Pastebin.checkBash).toHaveValue("Bash");
  });
});
